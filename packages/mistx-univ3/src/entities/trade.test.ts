import { Percent, Price, sqrt, Token, CurrencyAmount, TradeType, WETH, Ether } from '@alchemist-coin/mistx-core'
...
describe('Trade', () => {
  ...
  const pool_weth_0 = v2StylePool(
    CurrencyAmount.fromRawAmount(WETH[1], JSBI.BigInt(100000)),
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for ETHER currency input', async () => {
      ...
      expect(result[0].swaps[0].route.tokenPath).toEqual([WETH[1], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(ETHER)
      expect(result[1].swaps[0].route.tokenPath).toEqual([WETH[1], token0, token3])
    ...
    it('works for ETHER currency output', async () => {
      ...
      expect(result[0].swaps[0].route.tokenPath).toEqual([token3, token0, WETH[1]])
      expect(result[0].outputAmount.currency).toEqual(ETHER)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].swaps[0].route.tokenPath).toEqual([token3, token1, token0, WETH[1]])
