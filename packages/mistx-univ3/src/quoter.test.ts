...
import { CurrencyAmount, Token, TradeType, WETH } from '@alchemist-coin/mistx-core'
...
describe('SwapQuoter', () => {
  ...
  const pool_1_weth = makePool(token1, WETH[1])
  ...
  describe('#swapCallParameters', () => {
    describe('single trade input', () => {
      ...
      it('multi-hop exact input', async () => {
        const trade = await Trade.fromRoute(
          new Route([pool_0_1, pool_1_weth], token0, WETH[1]),
          CurrencyAmount.fromRawAmount(token0, 100),
          TradeType.EXACT_INPUT
        )
      ...
      it('multi-hop exact output', async () => {
        const trade = await Trade.fromRoute(
          new Route([pool_0_1, pool_1_weth], token0, WETH[1]),
          CurrencyAmount.fromRawAmount(WETH[1], 100),
          TradeType.EXACT_OUTPUT
        )
